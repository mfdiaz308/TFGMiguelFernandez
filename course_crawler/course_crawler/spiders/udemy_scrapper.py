    ...
    for url_tag2 in urls:
      ...
      for url_tag in tqdm(courses_urls, total=len(courses_urls), desc=str(i)+'/'+str(x)):
        ...
        try:      
          ...
          skills = []
          for s in skills_tag:
              skills.append(convert_to_utf8(s.string))
          ...
          try:
            ...
            description = convert_to_utf8(l.text)
          except:
            description = ''
          ...
          topics = []
          for t in topics_tag:
              topics.append(convert_to_utf8(t.string))
          ...
          outcomes = []
          for l in learnings:
            outcomes.append(convert_to_utf8(l.text))
          ...
          with open(f"spiders/course_crawler_data/udemy/{generate_hash(url_tag.text)}.json", 'w') as f:
            f.write(json_string)

        # If there is any problem, we catch the exception and ignore that url and go for the next one
        except:
          print(f'{str(j)}/{str(y)}|{str(i)}/{str(x)}: Unable to parse {url}')
          print('Ignoring url...')
            
    print(key + ' ended!')
    # We close the file
    f.close()
    fjson = open('udemy/' + 'course' + 's.json', 'w', encoding='UTF8', newline='')
